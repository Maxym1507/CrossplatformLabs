...
namespace LabsLibrary
{
    public static class Lab1
    {
        public static string Run(string pathInpFile = "INPUT.TXT")
        {
            ...
            List<(double, double)> prices = inputData.Skip(1)
                .Select(p => {
                    var vals = p.Split(new char[] { ' ' }, StringSplitOptions.RemoveEmptyEntries);
                    return (Convert.ToDouble(vals[0].Replace('.', ',')), Convert.ToDouble(vals[1].Replace('.', ',')));
                }).ToList();

            return string.Format("{0:0.00}", GetMaxEarnings(prices)).Replace(',', '.');
        }
